...
int waitPid(void){	
    ...
    printf(1, " - This is the parent: Child# %d has exited with status %d\n",ret_pid, exit_status);
    sleep(5);
    // If we try waiting for a process that doesn't exist...
    printf(1, " - this is parent, waiting for nonexistent child");
    waitpid(100, NULL, 0);
    printf(1, " - this is parent, stopped waiting for nonexistent child");
    printf(1, " - This is the parent: Now waiting for child with PID# %d\n",pid_a[5]);
    ret_pid = waitpid(pid_a[5], &exit_status, 0);
    printf(1, " - This is the parent: Child# %d has exited with status %d\n",ret_pid, exit_status);
